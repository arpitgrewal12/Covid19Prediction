...
def main():
	...
	# Convert the dates from "datetime" type to a float.
	#df['Last_Update'] = df['Last_Update'].values.astype(float)
	df['date_confirmation'] = df['date_confirmation'].values.astype(float)


	# ### Encoding "sex" attribute

	sexEncoder = LabelEncoder()
	# Fit the encoder to the values
	sexEncoder.fit(df['sex'].unique())
	...
	# Fit the encoder to the values
	outcomeEncoder.fit(df['outcome'].unique())
	df['outcome'] = outcomeEncoder.transform(df['outcome'])

	provinceLabelEncoder = LabelEncoder()

	# Fit the encoder to the values
	provinceLabelEncoder.fit(df['province'].unique())
	labelProvinces = provinceLabelEncoder.transform(df['province'])


	countryLabelEncoder = LabelEncoder()
	# Fit the encoder to the values
	countryLabelEncoder.fit(df['country'].unique())
	labelCountries = countryLabelEncoder.transform(df['country'])

	# Encode the province/countries.
	encodedDataset = pd.get_dummies(df, prefix=['province', 'country'], columns=['province', 'country'])

	# ## Feature Selection
	# At this point, we need to select some features to work with.

	tempDataset = df.copy()
	tempDataset['province'] = labelProvinces
	tempDataset['country'] = labelCountries
	tempDataset.head()


	print("Provinces in encoded dataset", len(tempDataset.province.unique()))
	print("Provinces in origina", len(df.province.unique()))

	# Extract class labels
	predictedClass = tempDataset['outcome'].copy() 
	features = tempDataset.drop('outcome', axis=1)

	mutualInfo = mutual_info_classif(features, predictedClass)

	for colName, score in zip(features.columns.to_list(), mutualInfo):
	    print("Score for", colName, "is:", score)


	# ### Observations
	# It seems like `age` and `sex` attributes are quite weak in terms of information gain which is a suprise. Perhaps I(Yavor) am setting a very high threshold but anything below `0.5` seems to be quite low to contribute. All columns below that threshold will be removed.
	# 
	# Another thing to notice here is that we have two sets of latitude/longitude coordinates. Keeping both could be seen under two different lights:
	# 1. Keeping both sets is redundant. This is since the first set(`latitude` and `longitude`) is for the specific province in question. On the other hand, `Lat` and `Long_` are pointing to either the entire country or province. 
	# 2. Keeping both sets gives more information.
	# 
	# For now, both sets of coordinates will be kept.

	# ## Drop columns contributing low information
	...
	for colName, score in zip(features.columns.to_list(), mutualInfo):
	    if score < 0.5:
	        print("Score for", colName, "will be dropped")
	        droplist.append(colName)



	# Drop all columns which scored below.
	# The step above was done for visual inspection
	for colName in droplist:
	    encodedDataset.drop(colName, axis=1,inplace=True)


	# ## Split the dataset into train/test samples
	# The data is split into an 80:20 ratio of training to test. There are two steps to this process:
	# 1. Split dataset into features/class label
	# 2. Split features/class labels into test/train subsets.
	# 
	# Since we are trying to predict the outcome of a case, we will use the `outcome` attribute as our class label. Every other attribute will therefore be a feature.

	# Extract class labels
	y = encodedDataset['outcome'].copy() 
	X = encodedDataset.drop('outcome', axis=1)

	X.columns.to_list() # These are the features.

	X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
	print("X_train shape=",X_train.shape)
	print("Normalized values=", encodedDataset['outcome'].value_counts(normalize=True))
